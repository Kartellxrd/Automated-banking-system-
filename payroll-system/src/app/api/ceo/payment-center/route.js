...
async function loadPaymentCenter(db) {
  const [payrollResult, expensesResult, runsResult, accountsResult] = await Promise.all([
    db.from('payroll_batches').select('id,batch_code,pay_period_id,status,total_employees,net_total,ceo_reviewed_at,submitted_at').eq('status', 'approved_by_ceo').order('ceo_reviewed_at'),
  ...
  const [entriesResult, periodsResult, sitesResult, categoriesResult, itemsResult, batchesResult, payeesResult, payoutProfilesResult, providersResult] = await Promise.all([
    ...
    batchIds.length ? db.from('payroll_batches').select('id,batch_code,pay_period_id,status,total_employees,net_total').in('id',batchIds) : Promise.resolve({data:[],error:null}),
  ...
  for (const entry of entriesResult.data||[]) { if(!entriesByBatch.has(entry.batch_id)) entriesByBatch.set(entry.batch_id,[]); entriesByBatch.get(entry.batch_id).push({...entry,net_pay:num(entry.net_pay)}); }
  const itemsByRun = new Map();
  for (const item of itemsResult.data||[]) { if(!itemsByRun.has(item.payment_run_id)) itemsByRun.set(item.payment_run_id,[]); itemsByRun.get(item.payment_run_id).push({...item,amount:num(item.amount)}); }
  ...
  for (const run of runs) {
    ...
    if (run.run_type==='expense') for (const item of itemsByRun.get(run.id)||[]) if (item.expense_request_id && !expenseRunByRequest.has(item.expense_request_id)) expenseRunByRequest.set(item.expense_request_id,run);
  }
  ...
  const payroll = payrollBatches.map((batch)=>{
    ...
    for (const e of payable) { const key=e.payout_provider_name_snapshot||'Missing payout provider'; const cur=breakdown.get(key)||{provider:key,recipients:0,amount:0}; cur.recipients+=1; cur.amount+=e.net_pay; breakdown.set(key,cur); }
    return {...batch,net_total:num(batch.net_total),pay_period:periodMap.get(batch.pay_period_id)||null,payable_recipients:payable.length,payment_blockers:blockers.length,payment_ready:payable.length>0&&blockers.length===0,payment_run:payrollRunByBatch.get(batch.id)||null,channel_breakdown:[...breakdown.values()].map((x)=>({...x,amount:Number(x.amount.toFixed(2))}))};
  });
  ...
  const expenses = expensesRaw.map((row)=>{
    ...
    return {...row,approved_amount:num(row.approved_amount),site:siteMap.get(row.site_id)||null,category:categoryMap.get(row.category_id)||null,payee:payee?{...payee,payout:profile?{...profile,provider,masked_destination:masked}:null}:null,payment_ready:ready,payment_run:expenseRunByRequest.get(row.id)||null,payment_blockers:ready?[]:[row.payee_id?'Assigned payee does not have a verified active payout profile.':'Accountant must assign a verified expense payee before payment.']};
  });

  const paymentRuns=runs.map((run)=>{const items=itemsByRun.get(run.id)||[];const counts=items.reduce((a,i)=>{a[i.status]=(a[i.status]||0)+1;return a;},{});const batch=run.payroll_batch_id?batchMap.get(run.payroll_batch_id)||null:null;return {...run,total_amount:num(run.total_amount),batch:batch?{...batch,net_total:num(batch.net_total),pay_period:periodMap.get(batch.pay_period_id)||null}:null,source_account:run.source_payment_account_id?accountMap.get(run.source_payment_account_id)||null:null,expense_request_id:items.find((i)=>i.expense_request_id)?.expense_request_id||null,progress:{queued:counts.queued||0,submitted:counts.submitted||0,paid:counts.paid||0,failed:counts.failed||0,skipped:counts.skipped||0}};});

  return { payroll, expenses, payment_runs: paymentRuns, summary:{ payroll_batches:payroll.length,payroll_recipients:payroll.reduce((s,r)=>s+r.payable_recipients,0),payroll_total:Number(payroll.reduce((s,r)=>s+r.net_total,0).toFixed(2)),payroll_ready_batches:payroll.filter((r)=>r.payment_ready&&!r.payment_run).length,expense_requests:expenses.length,expense_total:Number(expenses.reduce((s,r)=>s+r.approved_amount,0).toFixed(2)),expense_ready_requests:expenses.filter((r)=>r.payment_ready&&!r.payment_run).length,active_payment_runs:paymentRuns.filter((r)=>['prepared','executing','partial_failed','failed'].includes(r.status)).length,completed_payment_runs:paymentRuns.filter((r)=>r.status==='completed').length }, execution:{ mode:'manual_execution', message:'CEO authorization creates an immutable payment run. Finance then executes the real FNB bulk, Orange Money, P2C or eWallet transaction and records the actual reference. No gateway or fake success is assumed.' } };
}

export async function GET() {
  const access=await requireCEO('payments.view');
  if(!access.ok) return NextResponse.json({success:false,error:access.error},{status:access.status});
  try { const db=createSupabaseAdminClient(); return NextResponse.json({success:true,data:await loadPaymentCenter(db)}); }
  catch(error){ console.error('CEO payment center GET error:',error); return NextResponse.json({success:false,error:'Failed to load CEO payment center.'},{status:500}); }
}
...
export async function POST(request) {
  try {
    ...
    if(action==='prepare_payroll'){
      ...
      await writeAuditLog(db,{actorUserId:access.user.id,action:'AUTHORIZE_PAYROLL_PAYMENT_RUN',module:'Payments',entityType:'payment_run',entityId:run.id,details:`CEO authorized payroll payment run ${run.run_code}.`,metadata:{payroll_batch_id:batchId,total_items:run.total_items,total_amount:num(run.total_amount)}});
      return NextResponse.json({success:true,data:run,message:'Pay All authorized. Finance can now execute the real payment run.'},{status:201});
    }
    ...
    if(action==='prepare_expense'){
      ...
      if(error) return NextResponse.json({success:false,error:error.message},{status:400});
      await writeAuditLog(db,{actorUserId:access.user.id,action:'AUTHORIZE_EXPENSE_PAYMENT_RUN',module:'Payments',entityType:'payment_run',entityId:run.id,details:`CEO authorized expense payment run ${run.run_code}.`,metadata:{expense_request_id:requestId,total_amount:num(run.total_amount)}});
      return NextResponse.json({success:true,data:run,message:'Expense payment authorized. Finance can now execute it from the company payment account.'},{status:201});
    }

    return NextResponse.json({success:false,error:'Unsupported payment center action.'},{status:400});
  } catch(error){ console.error('CEO payment center POST error:',error); return NextResponse.json({success:false,error:'Failed to authorize payment.'},{status:500}); }
}
