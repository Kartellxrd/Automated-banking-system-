...
const LOCKED_ADMIN_PERMISSIONS = new Set([
  'admin.users.manage',
  'admin.sites.manage',
  'admin.site_assignments.manage',
...
export async function GET() {
  ...
  try {
    ...
    const [{ data: permissions, error: pError }, { data: grants, error: gError }] = await Promise.all([
      db.from('permissions')
        .select('id, permission_key, module, name, description, display_order')
        .eq('is_active', true)
        .order('module')
        .order('display_order'),
    ...
    return NextResponse.json({
      ...
      grants: grants || [],
      locked_admin_permissions: [...LOCKED_ADMIN_PERMISSIONS],
    });
  } catch (error) {
...
export async function PUT(request) {
  ...
  try {
    ...
    if (!changes.length) {
      return NextResponse.json({ success: false, error: 'No permission changes were supplied.' }, { status: 400 });
    }
