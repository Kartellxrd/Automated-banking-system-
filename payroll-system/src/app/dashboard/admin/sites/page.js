...
export default function AdminSitesPage() {
  ...
  async function handleToggleActive(site) {
    ...
    if (!window.confirm(`${verb.charAt(0).toUpperCase() + verb.slice(1)} ${site.site_name}?`)) return;
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok || !result.success) {
        throw new Error(result.error || `Failed to ${verb} site.`);
      }
      setMessage({ type: 'success', text: result.message });
      await fetchSites();
    } catch (error) {
  ...
  const filteredSites = useMemo(() => {
    ...
    return sites.filter((site) => {
      const matchesSearch =
        ...
        clerkName(site).toLowerCase().includes(q);
      const matchesStatus =
        statusFilter === 'ALL' ||
        (statusFilter === 'ACTIVE' && site.is_active !== false) ||
        (statusFilter === 'INACTIVE' && site.is_active === false);
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900 flex flex-col lg:flex-row">
      ...
      <main className="flex-1 p-4 sm:p-6 lg:p-8 space-y-6 overflow-x-hidden">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-visible">
          <div className="p-5 sm:p-6 border-b border-slate-200 flex flex-col xl:flex-row xl:items-center justify-between gap-4">
            <div>
              <h1 className="text-lg font-bold text-slate-950">Company Sites</h1>
              <p className="text-sm text-slate-500 mt-1">Sites are loaded from Supabase. No site names or locations are hardcoded in this screen.</p>
            </div>

            <div className="flex flex-col sm:flex-row gap-2">
              <div className="relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
                <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search sites..." className={`${inputClass} sm:w-56 pl-9`} />
              </div>
              ...
              <button onClick={fetchSites} className="inline-flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl border border-slate-200 bg-white text-sm font-semibold hover:bg-slate-50">
            ...
            <table className="w-full min-w-[850px] text-sm">
              <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                <tr>
                  ...
                  <th className="px-5 py-3 text-left">Assigned Clerk</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  <tr><td colSpan="6" className="px-5 py-12 text-center text-slate-500"><Loader2 className="h-5 w-5 animate-spin inline-block mr-2" />Loading sites...</td></tr>
                ) : filteredSites.length === 0 ? (
                  <tr><td colSpan="6" className="px-5 py-12 text-center text-slate-500">No sites found.</td></tr>
                  ...
                  <tr key={site.id} className="hover:bg-slate-50/70">
                    ...
                    <td className="px-5 py-4">
                      {site.assignment?.clerk ? (
                        <div>
                          <div className="font-medium text-slate-900">{clerkName(site)}</div>
                          <div className="text-xs text-slate-500">{site.assignment.clerk.email}</div>
                        </div>
                      ) : (
                    ...
                    <td className="px-5 py-4">
                      <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${site.is_active === false ? 'bg-slate-100 text-slate-600' : 'bg-emerald-50 text-emerald-700'}`}>
                    ...
                    <td className="px-5 py-4 text-right relative">
                      {actionLoadingId === site.id ? (
                        ...
                        <>
                          <button onClick={() => setOpenMenuId(openMenuId === site.id ? null : site.id)} className="p-2 rounded-lg border border-slate-200 hover:bg-slate-50" aria-label="Site actions">
                          ...
                          {openMenuId === site.id && (
                            <div className="absolute right-5 top-12 z-30 w-48 rounded-xl border border-slate-200 bg-white p-1.5 text-left shadow-xl">
                              <button onClick={() => openEditModal(site)} className="w-full flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold hover:bg-slate-50"><Edit3 className="h-4 w-4" />Edit Site</button>
                              <button onClick={() => handleToggleActive(site)} className="w-full flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold hover:bg-slate-50">
                                <Building2 className="h-4 w-4" />{site.is_active === false ? 'Reactivate Site' : 'Deactivate Site'}
                              </button>
                            </div>
                          )}
                        </>
      ...
      {showCreateModal && (
        <SiteModal
          title="Create Site"
          description="Create a company site. Clerk assignment is managed separately."
          form={form}
          setForm={setForm}
      ...
      {editTarget && (
        <SiteModal
          title="Edit Site"
          description={`Update ${editTarget.site_name}. Existing attendance history keeps the same site ID.`}
          form={editForm}
          setForm={setEditForm}
          onSubmit={handleEditSite}
          onClose={() => setEditTarget(null)}
          submitting={actionLoadingId === editTarget.id}
...
function MetricCard({ title, value, icon: Icon }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm flex items-center justify-between">
      <div>
        <p className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-500">{title}</p>
        <p className="mt-1 text-2xl font-bold text-slate-950">{value}</p>
      </div>
      <div className="rounded-xl border border-indigo-100 bg-indigo-50 p-3 text-indigo-600"><Icon className="h-5 w-5" /></div>
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-50 bg-slate-950/50 backdrop-blur-sm p-4 flex items-center justify-center">
      <div className="w-full max-w-lg rounded-2xl bg-white border border-slate-200 shadow-2xl">
        <div className="p-5 sm:p-6 border-b border-slate-200 flex items-start justify-between gap-4">
          ...
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-slate-100 text-slate-500"><X className="h-5 w-5" /></button>
        </div>
        <form onSubmit={onSubmit} className="p-5 sm:p-6 space-y-4">
          <label className="block">
            <span className="block text-xs font-bold text-slate-700 mb-1.5">Site name</span>
            <input required value={form.site_name} onChange={(e) => setForm((current) => ({ ...current, site_name: e.target.value }))} className={inputClass} placeholder="Enter site name" />
          </label>
          <label className="block">
            <span className="block text-xs font-bold text-slate-700 mb-1.5">Location</span>
            <input value={form.location} onChange={(e) => setForm((current) => ({ ...current, location: e.target.value }))} className={inputClass} placeholder="Enter physical location" />
          </label>
          <div className="pt-2 flex justify-end gap-2">
            <button type="button" onClick={onClose} className="px-4 py-2.5 rounded-xl border border-slate-200 text-sm font-semibold hover:bg-slate-50">Cancel</button>
            <button type="submit" disabled={submitting} className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-indigo-600 text-white text-sm font-semibold hover:bg-indigo-700 disabled:opacity-60">
              {submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />}{submitLabel}
            </button>
          </div>
        </form>
      </div>
