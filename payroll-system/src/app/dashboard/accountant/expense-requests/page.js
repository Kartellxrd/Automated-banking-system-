...
export default function AccountantExpenseRequestsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRequests(json.data || []); setSites(json.sites || []); setSummary(json.summary || {}); setOutstanding(json.outstanding_by_site || []);
      if (selected) setSelected((json.data || []).find((row) => row.id === selected.id) || null);
  ...
  async function runAction() {
    ...
    try {
      const res = await fetch('/api/accountant/expense-requests', {
        ...
        body: JSON.stringify({ id: selected.id, action, reason, recommended_amount: recommended === '' ? null : Number(recommended) }),
      });
      const json = await res.json();
  ...
  return <div className="min-h-screen bg-slate-100 flex flex-col lg:flex-row text-slate-900">
    ...
    <div className="flex-1 min-w-0"><AccNavbar title="Expense Requests & Reconciliation" subtitle="Review site funding requests, track advances, and reconcile company money" /><main className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <section className="rounded-3xl bg-white border border-slate-200 p-4 shadow-sm grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        <div className="flex gap-2"><select value={statusFilter} onChange={(e)=>setStatusFilter(e.target.value)} className="flex-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm font-semibold"><option value="all">All Statuses</option><option value="submitted">Submitted</option><option value="pending_ceo">Pending CEO</option><option value="funded">Funded</option><option value="reconciliation_submitted">Reconciliation Submitted</option><option value="reconciliation_issue">Reconciliation Issue</option><option value="reconciled">Reconciled</option><option value="rejected">Rejected</option></select><button onClick={load} className="rounded-xl border border-slate-200 px-3"><RefreshCw className={`w-4 h-4 ${loading?'animate-spin':''}`} /></button></div>
      </section>
    ...
    {selected && <div className="fixed inset-0 z-50 bg-slate-950/60 p-4 flex items-center justify-center"><div className="w-full max-w-4xl max-h-[94vh] overflow-y-auto rounded-3xl bg-white shadow-2xl"><div className="p-5 border-b border-slate-100 flex justify-between items-start"><div><div className="text-[10px] uppercase tracking-wider text-indigo-600 font-bold">{selected.request_code}</div><h2 className="text-xl font-black mt-1">{selected.purpose}</h2><p className="text-xs text-slate-500 mt-1">{selected.site?.site_name} • {selected.category?.category_name}</p></div><button onClick={()=>{setSelected(null);setAction(null);}} className="p-2 rounded-xl hover:bg-slate-100"><X className="w-5 h-5" /></button></div><div className="p-5 space-y-5">
      ...
      {selected.status==='submitted' && <div className="flex flex-wrap justify-end gap-2"><button onClick={()=>beginAction('reject')} className="rounded-xl border border-rose-200 bg-rose-50 text-rose-700 px-4 py-2.5 text-sm font-bold flex gap-2 items-center"><XCircle className="w-4 h-4" />Reject</button><button onClick={()=>beginAction('return')} className="rounded-xl border border-amber-200 bg-amber-50 text-amber-700 px-4 py-2.5 text-sm font-bold flex gap-2 items-center"><RotateCcw className="w-4 h-4" />Return for Correction</button><button onClick={()=>beginAction('send_to_ceo')} className="rounded-xl bg-indigo-600 text-white px-4 py-2.5 text-sm font-bold flex gap-2 items-center"><Send className="w-4 h-4" />Send to CEO</button></div>}
      {selected.status==='reconciliation_submitted' && <div className="flex justify-end gap-2"><button onClick={()=>beginAction('issue')} className="rounded-xl border border-rose-200 bg-rose-50 text-rose-700 px-4 py-2.5 text-sm font-bold">Flag Issue</button><button onClick={()=>beginAction('reconcile')} disabled={Math.abs(selected.unexplained_amount)>.01} className="rounded-xl bg-emerald-600 text-white px-4 py-2.5 text-sm font-bold disabled:bg-slate-300"><CheckCircle2 className="w-4 h-4 inline mr-1" />Mark Reconciled</button></div>}
      {selected.status==='pending_ceo' && <div className="rounded-2xl bg-indigo-50 border border-indigo-200 p-4 text-sm text-indigo-800 font-semibold">Waiting for CEO approval. Accountant cannot approve their own company outflow.</div>}
      ...
      {action && <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 space-y-3"><div className="font-black capitalize">{action.replaceAll('_',' ')}</div>{action==='send_to_ceo' && <label className="block"><span className="text-xs font-bold text-slate-600">Recommended Amount for CEO</span><input type="number" min="0.01" step="0.01" value={recommended} onChange={(e)=>setRecommended(e.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm" /></label>}{['return','reject','issue'].includes(action) && <label className="block"><span className="text-xs font-bold text-slate-600">Reason</span><textarea value={reason} onChange={(e)=>setReason(e.target.value)} rows={3} className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm" /></label>}<div className="flex justify-end gap-2"><button onClick={()=>setAction(null)} className="px-4 py-2 text-sm font-bold">Cancel</button><button disabled={saving || (action==='send_to_ceo' && Number(recommended)<=0) || (['return','reject','issue'].includes(action)&&!reason.trim())} onClick={runAction} className="rounded-xl bg-slate-900 text-white px-4 py-2 text-sm font-bold disabled:opacity-40">{saving?'Saving...':'Confirm'}</button></div></div>}
    </div></div></div>}
  </div>;
}
