...
import { AlertTriangle, ArrowLeft, CalendarDays, CheckCircle2, FileText, Loader2, RefreshCw, Send, WalletCards, XCircle } from 'lucide-react';
...
function label(status) {
  return {
    draft: 'Draft', ready_for_ceo: 'Waiting for CEO', rejected_by_ceo: 'Returned by CEO', approved_by_ceo: 'CEO Approved', executing: 'Payment Processing', paid: 'Paid', partial_failed: 'Partial Failure', failed: 'Failed',
  }[status] || status;
}
...
export default function PayrollBatchDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setPayDate(result.data?.scheduled_payment_date || '');
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col lg:flex-row text-slate-900">
      ...
      <div className="flex-1 min-w-0">
        <AccNavbar title="Payroll Batch Review" subtitle="Verify calculations, payout readiness and payday before CEO handoff" />
        <main className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
          ...
          {loading ? <div className="p-20 flex justify-center gap-2 text-slate-500"><Loader2 className="w-5 h-5 animate-spin text-indigo-600" />Loading payroll batch...</div> : batch && <>
            <section className="rounded-3xl bg-slate-950 text-white p-6 sm:p-8 flex flex-col xl:flex-row xl:items-center justify-between gap-6">
              <div><p className="text-[11px] uppercase tracking-[0.18em] font-bold text-indigo-300">{label(batch.status)}</p><h1 className="text-2xl sm:text-3xl font-black mt-1">{batch.batch_code}</h1><p className="text-sm text-slate-300 mt-2">{batch.pay_period?.period_name} • {batch.total_employees} employees • {batch.rosters.length} approved roster{batch.rosters.length === 1 ? '' : 's'}</p><p className="text-sm font-bold text-indigo-200 mt-2 inline-flex items-center gap-2"><CalendarDays className="w-4 h-4" />Scheduled payday: {dateLabel(batch.scheduled_payment_date)}</p>{batch.ceo_rejection_reason && <p className="mt-3 rounded-xl bg-rose-950/50 border border-rose-800 px-3 py-2 text-xs text-rose-200">CEO return reason: {batch.ceo_rejection_reason}</p>}</div>
            ...
            <section className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="rounded-3xl border border-slate-200 bg-white p-5"><div className="text-xs font-bold text-slate-500">Scheduled Payday</div><div className="text-lg font-black mt-2">{dateLabel(batch.scheduled_payment_date)}</div>{['draft','rejected_by_ceo'].includes(batch.status) ? <div className="mt-3 flex gap-2"><input type="date" value={payDate} onChange={(e) => setPayDate(e.target.value)} className="min-w-0 flex-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs" /><button onClick={() => act('set_pay_date', { scheduled_payment_date: payDate })} disabled={!payDate || working} className="rounded-xl bg-slate-900 px-3 py-2 text-xs font-bold text-white disabled:opacity-40">Save</button></div> : <p className="text-xs text-slate-500 mt-2">Locked after CEO submission.</p>}</div>
              <div className="rounded-3xl border border-slate-200 bg-white p-5"><div className="text-xs font-bold text-slate-500">Deductions</div><div className="text-3xl font-black mt-2">{money(batch.deductions_total)}</div><p className="text-xs text-slate-500 mt-2">Configured deductions only.</p></div>
            ...
            <section className="rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
              <div className="p-5 border-b border-slate-100 flex items-center justify-between"><div><h2 className="font-black flex items-center gap-2"><FileText className="w-5 h-5 text-indigo-600" />Employee Payroll Entries</h2><p className="text-xs text-slate-500 mt-1">Gross pay is built from approved regular and overtime hours using captured rate snapshots.</p></div><button onClick={() => act('refresh_payouts')} disabled={working || !['draft','rejected_by_ceo'].includes(batch.status)} className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold inline-flex items-center gap-2 disabled:opacity-40"><RefreshCw className={`w-4 h-4 ${working === 'refresh_payouts' ? 'animate-spin' : ''}`} />Refresh HR Payouts</button></div>
            ...
            <section className="rounded-3xl border border-indigo-100 bg-white p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4"><div className="flex items-start gap-3"><WalletCards className="w-5 h-5 text-indigo-600 mt-0.5" /><div><h3 className="font-black text-sm">Submit payroll to CEO</h3><p className="text-xs text-slate-500 mt-1">Submission requires verified payout details and a scheduled payday. The batch locks until CEO returns it.</p></div></div><button onClick={() => act('submit_to_ceo')} disabled={!batch.can_submit_to_ceo || working} className="rounded-xl bg-indigo-600 px-5 py-3 text-sm font-bold text-white disabled:opacity-40 inline-flex items-center justify-center gap-2">{working === 'submit_to_ceo' ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}Submit Batch to CEO</button></section>
          </>}
        </main>
      </div>
    </div>
