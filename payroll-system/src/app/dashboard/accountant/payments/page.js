...
export default function AccountantPaymentsPage() {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (!response.ok || !json.success) throw new Error(json.error || 'Could not load payment processing queue.');
      setRuns(json.data || []);
  ...
  async function startRun(event) {
    ...
    try {
      ...
      setSuccess(`${selected.run_code} is ready for bank/mobile submission.`);
      setStartOpen(false);
      await load(true);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col lg:flex-row text-slate-900">
      ...
      <div className="flex-1 min-w-0">
        <AccNavbar title="Payment Processing" subtitle="Submit CEO-released payroll and expenses to the approved payment channel" />
        <main className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
          <section className="grid grid-cols-2 xl:grid-cols-5 gap-4">
            <Metric label="Awaiting Submission" value={summary.awaiting_start || 0} />
          ...
          <section className="space-y-4">
            {loading ? <div className="p-16 text-center text-slate-500"><Loader2 className="w-6 h-6 animate-spin mx-auto mb-2" />Loading released payments...</div> : filtered.length === 0 ? <div className="rounded-3xl border border-slate-200 bg-white p-12 text-center text-slate-500">No payments match the current filter.</div> : filtered.map((run) => (
              <div key={run.id} className="rounded-3xl border border-slate-200 bg-white p-5 sm:p-6 shadow-sm space-y-4">
                <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
                  <div><div className="flex flex-wrap items-center gap-2"><h2 className="font-black text-lg">{run.run_code}</h2><Status value={run.status} /><span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-black uppercase text-slate-500">{run.run_type}</span></div><p className="text-sm text-slate-500 mt-1">{run.batch?.batch_code || (run.run_type === 'expense' ? 'Expense payment' : 'Payroll batch')} · {run.batch?.pay_period?.period_name || (run.run_type === 'expense' ? 'CEO released' : 'Pay period unavailable')} · CEO released {run.authorized_at ? new Date(run.authorized_at).toLocaleString('en-BW') : '—'}</p></div>
                  <div className="flex flex-wrap gap-2 items-center"><div className="mr-2"><div className="text-[10px] uppercase text-slate-400 font-bold">Payment Total</div><div className="text-xl font-black">{money(run.total_amount)}</div></div><button onClick={() => setSelected(run)} className="rounded-xl border border-slate-200 px-4 py-2.5 text-xs font-bold">View {run.total_items} Payment{run.total_items === 1 ? '' : 's'}</button>{run.status === 'prepared' && <button onClick={() => openStart(run)} disabled={!accounts.some((row) => row.is_active !== false)} className="rounded-xl bg-indigo-600 px-4 py-2.5 text-xs font-bold text-white disabled:bg-slate-300 inline-flex items-center gap-2"><PlayCircle className="w-4 h-4" />Start Processing</button>}<a href={`/api/accountant/payments/export?run_id=${encodeURIComponent(run.id)}`} className="rounded-xl bg-emerald-600 px-4 py-2.5 text-xs font-bold text-white inline-flex items-center gap-2"><Download className="w-4 h-4" />Payment Instruction CSV</a></div>
                </div>
                ...
                {run.status === 'prepared' && !accounts.some((row) => row.is_active !== false) && <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs font-semibold text-amber-800">Add the company payment source before processing this released payment.</div>}
              </div>
            ))}
          </section>
        </main>
      ...
      {selected && <div className="fixed inset-0 z-50 bg-slate-950/60 p-4 overflow-y-auto"><div className="max-w-6xl mx-auto my-5 rounded-3xl bg-white shadow-2xl"><div className="p-5 border-b border-slate-100 flex justify-between gap-4"><div><div className="text-xs uppercase tracking-wider font-black text-indigo-600">Released Payment</div><h2 className="text-xl font-black mt-1">{selected.run_code}</h2><p className="text-xs text-slate-500 mt-1">{selected.batch?.batch_code || selected.run_type} · {money(selected.total_amount)}</p></div><button onClick={() => setSelected(null)} className="p-2 rounded-xl hover:bg-slate-100"><X className="w-5 h-5" /></button></div><div className="p-5 overflow-x-auto"><table className="w-full min-w-[1000px] text-sm"><thead className="bg-slate-50 text-[10px] uppercase tracking-wider text-slate-400"><tr><th className="px-4 py-3 text-left">Payee</th><th className="px-4 py-3 text-left">Destination</th><th className="px-4 py-3 text-left">Amount</th><th className="px-4 py-3 text-left">Status</th><th className="px-4 py-3 text-left">Reference / Error</th><th className="px-4 py-3 text-right">Action</th></tr></thead><tbody className="divide-y divide-slate-100">{selected.items.map((item) => <tr key={item.id}><td className="px-4 py-4"><div className="font-bold">{item.payee_name_snapshot}</div><div className="text-[11px] text-slate-400">{item.payout_provider_name_snapshot || 'Provider not labelled'}</div></td><td className="px-4 py-4"><div className="font-mono text-xs">{item.destination_masked || '—'}</div><div className="text-[11px] text-slate-400">Branch {item.branch_code_snapshot || '—'}</div></td><td className="px-4 py-4 font-black">{money(item.amount)}</td><td className="px-4 py-4"><Status value={item.status} /></td><td className="px-4 py-4"><div className="text-xs font-semibold">{item.payment_reference || '—'}</div>{item.payment_error && <div className="text-[11px] text-rose-600 mt-1">{item.payment_error}</div>}</td><td className="px-4 py-4 text-right"><div className="inline-flex gap-1.5">{['queued','failed'].includes(item.status) && selected.status === 'executing' && <button onClick={() => beginItemAction(item, 'submitted')} className="rounded-lg bg-indigo-50 px-2.5 py-2 text-[11px] font-bold text-indigo-700"><Send className="w-3.5 h-3.5 inline mr-1" />Submitted to Bank</button>}{['queued','submitted','failed'].includes(item.status) && selected.status === 'executing' && <button onClick={() => beginItemAction(item, 'paid')} className="rounded-lg bg-emerald-50 px-2.5 py-2 text-[11px] font-bold text-emerald-700"><CheckCircle2 className="w-3.5 h-3.5 inline mr-1" />Confirm Paid</button>}{['queued','submitted'].includes(item.status) && selected.status === 'executing' && <button onClick={() => beginItemAction(item, 'failed')} className="rounded-lg bg-rose-50 px-2.5 py-2 text-[11px] font-bold text-rose-700"><XCircle className="w-3.5 h-3.5 inline mr-1" />Failed</button>}{item.status === 'failed' && ['partial_failed','failed','executing'].includes(selected.status) && <button onClick={() => beginItemAction(item, 'retry')} className="rounded-lg bg-amber-50 px-2.5 py-2 text-[11px] font-bold text-amber-700"><RotateCcw className="w-3.5 h-3.5 inline mr-1" />Retry</button>}</div></td></tr>)}</tbody></table></div></div></div>}

      {startOpen && selected && <Modal title="Start Payment Processing" subtitle={`${selected.run_code} · ${money(selected.total_amount)}`} onClose={() => !saving && setStartOpen(false)}><form onSubmit={startRun} className="space-y-4"><label className="block"><span className="text-xs font-bold text-slate-600">Company payment source *</span><select required value={startForm.source_account_id} onChange={(e) => setStartForm({ ...startForm, source_account_id: e.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm"><option value="">Select payment source</option>{accounts.filter((row) => row.is_active !== false).map((row) => <option key={row.id} value={row.id}>{row.account_name} — {row.institution_name}{row.account_identifier_label ? ` (${row.account_identifier_label})` : ''}</option>)}</select></label><label className="block"><span className="text-xs font-bold text-slate-600">Processing notes</span><textarea value={startForm.notes} onChange={(e) => setStartForm({ ...startForm, notes: e.target.value })} rows={3} className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm" placeholder="Optional notes for this payment cycle" /></label><div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">Starting processing selects the company source account and opens the released instructions for submission. It does not mark a recipient paid.</div><Actions busy={saving} onCancel={() => setStartOpen(false)} label="Start Processing" /></form></Modal>}

      {accountOpen && <Modal title="Add Company Payment Source" subtitle="Store a safe source label only. Bank login credentials are never stored here." onClose={() => !saving && setAccountOpen(false)}><form onSubmit={createAccount} className="space-y-4"><Field label="Account label *" value={accountForm.account_name} onChange={(value) => setAccountForm({ ...accountForm, account_name: value })} placeholder="e.g. Main Payroll Account" /><Field label="Institution *" value={accountForm.institution_name} onChange={(value) => setAccountForm({ ...accountForm, institution_name: value })} placeholder="e.g. First National Bank Botswana" /><label className="block"><span className="text-xs font-bold text-slate-600">Account type</span><select value={accountForm.account_type} onChange={(e) => setAccountForm({ ...accountForm, account_type: e.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm"><option value="bank">Bank</option><option value="mobile_wallet">Mobile Wallet</option><option value="other">Other</option></select></label><Field label="Safe identifier" value={accountForm.account_identifier_label} onChange={(value) => setAccountForm({ ...accountForm, account_identifier_label: value })} placeholder="e.g. ending 4821 (optional)" /><label className="block"><span className="text-xs font-bold text-slate-600">Notes</span><textarea value={accountForm.notes} onChange={(e) => setAccountForm({ ...accountForm, notes: e.target.value })} rows={2} className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm" /></label><Actions busy={saving} onCancel={() => setAccountOpen(false)} label="Add Payment Source" /></form></Modal>}

      {itemAction && <Modal title={itemAction.action === 'retry' ? 'Retry Failed Payment' : itemAction.action === 'submitted' ? 'Record Bank Submission' : itemAction.action === 'paid' ? 'Confirm Settled Payment' : 'Record Failed Payment'} subtitle={`${itemAction.item.payee_name_snapshot} · ${money(itemAction.item.amount)}`} onClose={() => !saving && setItemAction(null)}><form onSubmit={itemAction.action === 'retry' ? (e) => { e.preventDefault(); retryItem(); } : saveItemAction} className="space-y-4">{itemAction.action !== 'retry' && <><label className="block"><span className="text-xs font-bold text-slate-600">Payment method *</span><select required value={itemForm.execution_method_id} onChange={(e) => setItemForm({ ...itemForm, execution_method_id: e.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm"><option value="">Select method</option>{methods.map((method) => <option key={method.id} value={method.id}>{method.name}</option>)}</select></label>{itemAction.action !== 'failed' && <Field required label="Bank / transaction reference *" value={itemForm.reference} onChange={(value) => setItemForm({ ...itemForm, reference: value })} placeholder="Reference from FNB / OM / P2C / eWallet" />}{itemAction.action === 'failed' && <><Field label="Reference (if one exists)" value={itemForm.reference} onChange={(value) => setItemForm({ ...itemForm, reference: value })} /><label className="block"><span className="text-xs font-bold text-slate-600">Failure reason *</span><textarea required value={itemForm.error} onChange={(e) => setItemForm({ ...itemForm, error: e.target.value })} rows={3} className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm" /></label></>}</>}{itemAction.action === 'retry' && <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">The failed instruction returns to the processing queue. Paid recipients are not included again.</div>}<Actions busy={saving} onCancel={() => setItemAction(null)} label={itemAction.action === 'retry' ? 'Queue Retry' : 'Save Result'} /></form></Modal>}
    </div>
  );
}
...
function Status({ value }) { const good = ['completed','paid'].includes(value); const bad = ['failed','partial_failed'].includes(value); const label = value === 'prepared' ? 'released' : value === 'executing' ? 'processing' : pretty(value); return <span className={`rounded-full px-2.5 py-1 text-[10px] font-black uppercase ${good ? 'bg-emerald-50 text-emerald-700' : bad ? 'bg-rose-50 text-rose-700' : value === 'executing' || value === 'submitted' ? 'bg-indigo-50 text-indigo-700' : 'bg-amber-50 text-amber-700'}`}>{label}</span>; }
