...
import { Building2, CalendarDays, ChevronRight, ClipboardCheck, FileText, LayoutDashboard, LogOut, Menu, Users, X } from 'lucide-react';
      ...
      {navItems.map((item) => {
        ...
        return (
          <Link
            ...
            className={`flex items-center justify-between px-4 py-3 rounded-2xl text-sm font-bold transition ${active ? 'bg-indigo-600 text-white shadow-md shadow-indigo-600/20' : 'text-slate-400 hover:text-white hover:bg-slate-800/70'}`}
          >
    ...
    <>
      <div className="lg:hidden sticky top-0 z-40 bg-slate-950 text-white p-4 flex items-center justify-between border-b border-slate-800">
        <div className="flex items-center gap-3">
          <div className="p-2.5 bg-indigo-600 rounded-2xl"><Building2 className="w-5 h-5" /></div>
          <div><div className="font-black text-sm">Periscope Mining</div><div className="text-[10px] uppercase tracking-wider font-bold text-indigo-300">HR Portal</div></div>
        </div>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-slate-950 text-slate-300 p-6 flex flex-col justify-between transition-transform ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="space-y-7">
          <div className="flex items-center gap-3 border-b border-slate-800 pb-5">
            ...
            <div><h2 className="font-black text-white">Periscope Mining</h2><p className="text-[10px] uppercase tracking-[0.16em] font-bold text-indigo-300">HR & Workforce</p></div>
          </div>
          {links}
          <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4 text-xs leading-5 text-slate-400">
            <span className="font-bold text-slate-200">HR ownership</span><br />Employees → documents → attendance approval → payroll-ready data.
          </div>
        </div>
